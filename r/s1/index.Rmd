...
Lets download some data that we can use for this workshop series: [FABI Workshop Series >> Mock Data](http://jmhulbert.github.io/r/data)
...
##### Outline
Another nice feature in RStudio is the ability to create an outline (psuedo table of contents). Click 'show document outline' in top right corner of the Script Pane.
...
##### Assign variables 
As we saw before you can assign variables by using <- or =. I prefer to use <- so that I do not confuse the = with == (more later)
...
Anytime we want to refer to a single column or variable we can do so using the $ symbol.
...
We can look at teh structure of a dataset and check how R reads variables in a dataset using the str() function
...
In most cases, it is easiest to change misspelt observations and clean our data in excel using the 'Find and replace' functions. However, it is safer to do it in R, because it is easy to make mistakes in Excel. 

For example, if you have a lot of levels because it was misspelt a few times, you can use the stringr package to change them with the below code.
...
We will learn more about cleaning our data in the [Session 3 - Introduction to Dyplr](http://jmhulbert.github.io/r/s3). There we will learn to manipulate, transform, and summarize our data. 
