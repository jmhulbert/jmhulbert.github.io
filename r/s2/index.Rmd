...
You will need to have RStudio (and R) installed on your computer and have the 'tidyverse' package installed. More information about this can be found in the lesson for [Session 1](http://jmhulbert.github.io/r/s1).
...
Today we are going to learn to use ggplot, but we will learn to use dplyr next week in [Session 3](http://jmhulbert.github.io/r/s1). Knowing how to use both of these packages provides a strong foundation for analyses. Today we will learn to plot raw data with ggplot, but next week we will be able to summarize (calculate means, standard errors, add variables, etc.), then plot the summarized data.
...
> Shoot I forgot about that super outlier.. Looks like we will need a quick glimpse into dplyr ( [Session 3](http://jmhulbert.github.io/r/s3)). 
...
Remember that at least 10 measurements were made for each plant, so these observations do not represent each plant. We will learn to summarize (average) the observations per plant in [Session 3](http://jmhulbert.github.io/r/s3). 
...
Note that boxplots show the median, not the mean. We will learn to calculate and visualize means and standard errors in [Session 3](http://jmhulbert.github.io/r/s3).
...
* There are a few things that we don't have time for in this session but we can include in [Session 3](http://jmhulbert.github.io/r/s3). 
    + Labels
    + Add Text to plots
    + Change colors
    + Themes
